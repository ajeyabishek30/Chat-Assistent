...
export default function Home() {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    const minLoadingTime = 1000; // Minimum 1 second to show typing indicator
    ...
    try {
      ...
      if (error.message?.includes('Failed to fetch') || error.message?.includes('NetworkError') || error.name === 'TypeError') {
        ...
        // Ensure typing indicator shows for minimum time (simulate thinking)
        const elapsed = Date.now() - startTime;
        const remainingTime = Math.max(minLoadingTime - elapsed, 800); // At least 800ms more
        ...
        // Other errors - ensure minimum loading time then show error
        const elapsed = Date.now() - startTime;
        if (elapsed < minLoadingTime) {
          await new Promise(resolve => setTimeout(resolve, minLoadingTime - elapsed));
        }
